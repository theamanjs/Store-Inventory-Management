      ...
      <div class="navbar-nav ml-auto">
        ...
        <li class="nav-item dropdown">
          ...
          <div class="dropdown-menu" aria-labelledby="navbarDropdown">
            ...
            <div class="dropdown-divider"></div>
            <a class="dropdown-item bg-primary" href="#">Vendor</a>
          </div>
        </li>
	...
	<div class="concontent-item">
		<div class="concontent-item-enter">
			<div class="concontent-item-enter-data">
                <!-- vendor adding form starts -->
                <form method="POST">
            <legend>Vendor Insert</legend>
		<div class="form-group">
          <label for="items">Vendor's Name</label>
          <input type="text" onchange="upperCase(this)" class="form-control" id="items" name="name" placeholder="Enter Vendor name" >
        </div>
        <div class="form-group">
          <label for="items">Vendor's Deal Purpose</label>
          <input type="text" onchange="upperCase(this)" class="form-control" id="items" name="deal" placeholder="Vendor Dealing Purpose" >
        </div>
        <div class="form-group">
          <label for="items">GST Number</label>
          <input type="text" onchange="upperCase(this)" class="form-control" id="items" name="gstNumber" placeholder="Vendor GST Number" >
        </div>
        <div class="form-group">
        <label for="items">Vendor's Address</label>
          <textarea onchange="upperCase(this)" class="form-control" id="items" name="address" placeholder="Enter Address here" >
          </textarea>
        </div>
        <button type="submit" class="btn btn-primary consubb" name="vendor" onclick="return inventoryValidation()">Submit</button>
        </form>
            <!-- vendor adding form ends -->
<?php
if(isset($_POST['vendor'])){
$vendorName= $_POST['name'];
$vendorDeal= $_POST['deal'];
$gstNumber= $_POST['gstNumber'];
$vendorAddress = $_POST['address'];
$vendorInsert = "INSERT INTO vendor VALUE('','$vendorName','$gstNumber','$vendorDeal','$vendorAddress')";
$vendorInsertQuery = mysqli_query($conn,$vendorInsert);
if($vendorInsertQuery){
    echo '<script>alert("Vendor is added")</script>';
    // echo '<script>confirm("Do you want to add more vendor?")</script>';
}
else{
    echo '<script>alert("Vendor is not added !")</script>';
}
}
?>
</div>
...
<div class="vendorDisplay">
    ...
    <div class="form-group">
          ...
          <select class="custom-select" size="8" name="itemcate" id="vendor">
<?php
...
while($getVendorInfo = mysqli_fetch_array($getVendorQuery)){
    echo ' <option value="' .$getVendorInfo[1].'">'.$getVendorInfo[1].'</option> ';
}
?>
        </select>
        </div>
